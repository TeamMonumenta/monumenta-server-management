	...
	private void handleLoginPacket(PacketReader reader) {
		...
		switch (id) {
			case Protocol.SERVERBOUND_LOGIN_START -> {
				if (protocol == null) {
					throw ProtocolError.of("unsupported minecraft version");
				}
				...
				final var copyLoginInfo = new LoginInfo(
					reader.str(16),
					reader.uuid()
				);
				...
				write(writer -> {
					...
					writer.uuid(copyLoginInfo.uuid());
					writer.str(copyLoginInfo.name());

					writer.varInt(0); // PlayerInfoUpdate AddPlayer Property (empty list)

					if (protocol().inRange(Protocol.V1_20_5, Protocol.V1_21)) {
						writer.flag(false); // Strict Error Handling (surely we don't send any malformed packets flowey :3)
					}
				});
			}
			...
			case Protocol.SERVERBOUND_LOGIN_PLUGIN_MESSAGE -> {
				// NOOP - drop modern forwarding velocity verification confirmation packet
			}
			default -> ProtocolError.of("unknown packet in %s: %s", phase, id);
		}
	}
